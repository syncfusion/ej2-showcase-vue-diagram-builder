...
export abstract class MindMapUtilityMethods {
    ...
    private static updateLevel(parentNode: Node, level: number, orientation: string): void {
        ...
        for (let i: number = parentNode.outEdges.length - 1; i >= 0; i--) {
            ...
            if (childNode.outEdges.length > 0) {
                this.updateLevel(childNode, (parentNode.addInfo as any).level, orientation);
            } else {
                (childNode.addInfo as INodeInfo).level = (parentNode.addInfo as any).level + 1;
    ...
    public static onHideNodeClick(): void {
        let node1: NodeModel = MindMapUtilityMethods.getNode((this.selectedItem as any).selectedDiagram.nodes, 'textNode');
    ...
    public static getMindMapShape(parentNode: NodeModel): { [key: string]: Object } {
        ...
        if (addInfo.level < 1) {
            (node as any).style.fill = this.selectedItem.utilityMethods.fillColorCode[this.lastFillIndex];
            (node as any).style.strokeColor = this.selectedItem.utilityMethods.borderColorCode[this.lastFillIndex];;
            ...
            (node as any).style.strokeColor = (node as any).style.fill = (parentNode as any).style.fill;
        }
    ...
    public static setConnectorDefault(diagram: Diagram, orientation: string, connector: ConnectorModel, sourceID: string, targetID: string): ConnectorModel {
