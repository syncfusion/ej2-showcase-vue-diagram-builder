...
export class OrgChartData {
    ...
    private addRightChild(args: { [key: string]: Object }): void {
        ...
        if ((diagram as any).selectedItems.nodes.length > 0) {
            ...
            if (node.inEdges.length > 0) {
                let connector1: Connector = this.getConnector((diagram as any).connectors, node.inEdges[0]);
    ...
    public changeChildParent(): void {
        ...
        if (selectedNode.inEdges.length > 0) {
            let connector1: Connector = this.getConnector((diagram as any).connectors, selectedNode.inEdges[0]);
            let parentNode: Node = this.getNode((diagram as any).nodes, connector1.sourceID);
            if (parentNode.inEdges.length > 0) {
                let connector2: Connector = this.getConnector((diagram as any).connectors, parentNode.inEdges[0]);
    ...
    private navigateChild(direction: string): void {
        ...
        if (direction === 'left' || direction === 'right') {
            ...
            let index: number = sameLevelNodes.indexOf((diagram as any).selectedItems.nodes[0] as Node);
    ...
    private getSameLevelNodes(): Node[] {
        ...
        if ((diagram as any).selectedItems.nodes.length > 0) {
            let node: Node = (diagram as any).selectedItems.nodes[0] as Node;
            let connector: Connector = this.getConnector((diagram as any).connectors, node.inEdges[0]);
            let parentNode: Node = this.getNode((diagram as any).nodes, connector.sourceID);
            for (let i: number = 0; i < parentNode.outEdges.length; i++) {
                connector = this.getConnector((diagram as any).connectors, parentNode.outEdges[i]);
                let childNode: Node = this.getNode((diagram as any).nodes, connector.targetID);
                if (childNode) {
...
export abstract class OrgChartUtilityMethods {
    ...
    public static addChild(sourceId: string): void {
        ...
        connector.constraints = ConnectorConstraints.PointerEvents | ConnectorConstraints.Select | ConnectorConstraints.Delete;
        (diagram as any).add(connector);
        let node1: NodeModel = this.getNode((diagram as any).nodes, (node as any).id);
        (diagram as any).doLayout();
        (diagram as any).endGroupAction();
        this.selectedItem.preventPropertyChange = true;
    ...
    public static createEmptyOrgChart(): void {
        let diagram: Diagram | undefined = this.selectedItem.selectedDiagram;
        (diagram as any).constraints = (diagram as any).constraints & ~DiagramConstraints.UndoRedo;
        let node: NodeModel = {
        ...
        (diagram as any).add(node);
        let node1: NodeModel = {
            id: 'textNode', width: 400, height: 300, offsetX: (diagram as any).scrollSettings.viewPortWidth - 200,
        ...
        diagram?.add(node1);
        ((document.getElementById('diagram') as any).querySelector('#closeIconDiv') as HTMLElement).onclick = this.onHideNodeClick.bind(this);
        (diagram as any).constraints = (diagram as any).constraints | DiagramConstraints.UndoRedo;
    }

    public static onHideNodeClick(): void {
        let node1: NodeModel = MindMapUtilityMethods.getNode((this.selectedItem.selectedDiagram as any).nodes, 'textNode');
    ...
    public static applyDataSource(): void {
        ...
        (diagram as any).selectedItems = { userHandles: OrgChartUtilityMethods.handle, constraints: SelectorConstraints.UserHandle };
